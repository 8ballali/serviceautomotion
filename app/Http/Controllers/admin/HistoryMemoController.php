...
class HistoryMemoController extends Controller
{
    public function index($memo_id){
        $logged_in = Auth::id();
        $history = HistoryMemo::where('memo_id', $memo_id)->orderBy('created_at', 'DESC')->get();
    ...
    {
        $history = HistoryMemo::find($memo_id);
    ...
    {
        $download = HistoryMemo::where('id', $memo_id)->firstOrFail();
